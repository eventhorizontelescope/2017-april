...
def pipeline(
    ...
    # Pre Flag Table
    # -------------------------------------------------------------------------
    if prefgtab is not None:
        # Load prefgtab
        fgtab=pd.read_csv(prefgtab)[fgtab_columns]
        ehtfgin(msortdata, fgtab, outfgver=msortdata.table_highver("FG")+1)
        ...
        task.outtext=os.path.join(workdir,"%s.%02d.fg.preload.tbout"%(fileheader,fileid))
        task.docrt=-1
        ...
        fgtab.to_csv(os.path.join(workdir,"%s.%02d.fg.preload.csv"%(fileheader,fileid)))
        fileid += 1
    ...
    ehtacscl(
        ...
        doband=5,
        bpver=-1,
        flagmode=flagmode # 1: run SNEDT, 2: run EDITA and flag Data based on SN table
    )
    ...
    if interactive:
        ...
        task.inext='FG'
        task.inver=msortdata.table_highver("FG")
        task.outtext=os.path.join(workdir,"%s.%02d.fg.acscl1.tbout"%(fileheader,fileid))
        task.docrt=-1
        task.check(overwrite=True)
        task()
        fileid += 1
        ...
        fgtab=ehtfgout(msortdata)
        fgtab.to_csv(os.path.join(workdir,"%s.%02d.fg.acscl1.csv"%(fileheader,fileid)))
        fileid += 1
    ...
    ehtsnplt(
        ...
        optypes="AMP",
        outfile=os.path.join(workdir,"%s.%02d.acscl1.sn.if10.ps"%(fileheader,fileid)),
        nplots=Nant,
        overwrite=True,
        zappltabs=True)
    fileid += 1
    ...
    ehtsnplt(
        ...
        outfile=os.path.join(workdir,"%s.%02d.acscl1.cl.if10.ps"%(fileheader,fileid)),
        nplots=Nant,
        overwrite=True,
        zappltabs=True)
    fileid += 1
    ...
    ehtvplotac(
        ...
        outfile=os.path.join(workdir,"%s.%02d.acscl1.vplot.ac.if10.ps"%(fileheader,fileid)),
        overwrite=True,
        zappltabs=True)
    fileid += 1
    ...
    task.dataout=os.path.join(workdir,"%s.%02d.splat1.fittp"%(fileheader,fileid))
    task.check(overwrite=True)
    task()
    fileid += 1
    ...
    # -------------------------------------------------------------------------
    # Number of Antennas
    Nant=len(splat1data.antennas)
    Nch=splat1data.header["naxis"][2]
    Nif=splat1data.header["naxis"][3]

    # Check if SR and SM exists
    isSR = "SR" in splat1data.antennas
    isSM = "SM" in splat1data.antennas

    # Station Code
    AAid=splat1data.antennaids("AA")
    if isSR:
        SRid=splat1data.antennaids("SR")
    if isSM:
        SMid=splat1data.antennaids("SM")

    # Non ALMA station IDs
    NoAAs=splat1data.antennas
    NoAAs.remove("AA")
    NoAAs=splat1data.antennaids(NoAAs)
    NoAAs.insert(0, None)
    ...
    ehtsnplt(
        ...
        pltifs=10,
        inext="CL",
        invers=splat1data.table_highver("CL"),
    ...
    ehtpossmcc(
        indata=splat1data,
        docalib=1,
        gainuse=splat1data.table_highver("CL"),
        doband=-1,
        bpver=-1,
        solint=-1,
        bindif=True,
    ...
    if isSR and isSM:
        ...
        task.outfgver=splat1data.table_highver("FG")+1
        task.opcode="FLAG"
        ...
        task.getn(splat1data)
        task.inext='FG'
        task.inver=splat1data.table_highver("FG")
    ...
    # -------------------------------------------------------------------------
    tables=splat1data.tables
    task=tget("kring")
    task.getn(splat1data)
    task.docalib=1
    task.gainuse=splat1data.table_highver("CL")
    task.refant=AAid
    task.search[1]=AAid
    ...
    task.cparm[1]=0.4     # Data integration time: 0.4sec (see DTSUM)
    ...
    ehtsnplt(
        ...
        pltifs=10,
        inext="SN",
        invers=splat1data.table_highver("SN"),
        antennas=NoAAs,
    ...
    task.interpol="SELF"
    task.cutoff=10
    task.snver=splat1data.table_highver("SN")
    task.gainver=splat1data.table_highver("CL")
    task.gainuse=splat1data.table_highver("CL")+1
    task.refant=AAid
    task()
    ...
    ehtpossmcc(
        ...
        gainuse=splat1data.table_highver("CL"),
        doband=5,
        bpver=splat1data.table_highver("BP"),
        solint=-1,
        bindif=True,
        plotbp=True,
        outfile=os.path.join(workdir,"%s.%02d.bpasscc.bp.ps"%(fileheader,fileid)),
        pagetype=0,
        overwrite=True,
        zappltabs=True)
    fileid += 1
    ...
    ehtpossmcc(
        ...
        bindif=True,
        plotbp=False,
        outfile=os.path.join(workdir,"%s.%02d.bpasscc.cc.ps"%(fileheader,fileid)),
        pagetype=0,
        overwrite=True,
        zappltabs=True)
    fileid += 1
    ...
    ehtvplotac(
        ...
        doband=5,
        bpver=1,
        outfile=os.path.join(workdir,"%s.%02d.bpasscc.vplot.ac.if10.ps"%(fileheader,fileid)),
        overwrite=True,
        zappltabs=True)
    fileid += 1
    ...
    ehtpossmac(
        ...
        doband=5,
        bpver=1,
        outfile=os.path.join(workdir,"%s.%02d.bpasscc.possm.ac.ps"%(fileheader,fileid)),
        overwrite=True,
        zappltabs=True)
    fileid += 1
    ...
    if isSR and isSM:
        ...
        task.antennas[1]=int(np.min([SRid,SMid]))
        task.baseline[1]=int(np.max([SRid,SMid]))
        task.outfgver=splat1data.table_highver("FG")
        task.opcode="UFLG"
        task.reason="UNFLAG SMAP/SMAR BL"
        task.dohist=1
        task()
        ...
        task.inext='FG'
        task.inver=splat1data.table_highver("FG")
        task.outtext=os.path.join(workdir,"%s.%02d.fg.unflag_sma.tbout"%(fileheader,fileid))
        task.docrt=-1
        task.check(overwrite=True)
        task()
        fileid += 1
        ...
        fgtab=ehtfgout(splat1data)
        fgtab.to_csv(os.path.join(workdir,"%s.%02d.fg.unflag_sma.csv"%(fileheader,fileid)))
        fileid += 1

    
    # -------------------------------------------------------------------------
    # Third ACSCL: Correct small drifts in AC power induced by BPASS
    # -------------------------------------------------------------------------
    # 3rd ACSCL
    tables=splat1data.tables
    ehtacscl(
        splat1data,
        solint=1e-6,
        gainver=splat1data.table_highver("CL"),
        gainuse=splat1data.table_highver("CL")+1,
        doband=5,
        bpver=1,
    ...
    ehtsnplt(
        ...
        invers=splat1data.table_highver("SN"),
        optypes="AMP",
        outfile=os.path.join(workdir,"%s.%02d.acscl3.sn.if10.ps"%(fileheader,fileid)),
        nplots=Nant,
        overwrite=True,
        zappltabs=True)
    fileid += 1
    ...
    ehtsnplt(
        ...
        invers=splat1data.table_highver("CL"),
        optypes="AMP",
        outfile=os.path.join(workdir,"%s.%02d.acscl3.cl.if10.ps"%(fileheader,fileid)),
        nplots=Nant,
        overwrite=True,
        zappltabs=True)
    fileid += 1
    ...
    ehtvplotac(
        ...
        doband=5,
        bpver=1,
        outfile=os.path.join(workdir,"%s.%02d.acscl3.vplot.ac.if10.ps"%(fileheader,fileid)),
        overwrite=True,
        zappltabs=True)
    fileid += 1
    ...
    ehtpossmac(
        ...
        doband=5,
        bpver=1,
        outfile=os.path.join(workdir,"%s.%02d.acscl3.possm.ac.ps"%(fileheader,fileid)),
        overwrite=True,
        zappltabs=True)
    fileid += 1

    # Frequency vs (Scan-averaged) CC Spectrum: Whole IFs
    ehtpossmcc(
        indata=splat1data,
        docalib=1,
        gainuse=splat1data.table_highver("CL"),
        ...
        bindif=True,
        plotbp=False,
        outfile=os.path.join(workdir,"%s.%02d.acscl3.cc.ps"%(fileheader,fileid)),
        pagetype=0,
        overwrite=True,
        zappltabs=True)
    fileid += 1
